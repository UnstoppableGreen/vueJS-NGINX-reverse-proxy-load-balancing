      ...
      <div class="py-8 md:py-4">
        ...
        <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-6 overflow-x-auto">
          <div class="flex justify-between">
            <div>
              <p class="text-3xl font-bold">Командировки</p>
            </div>
          ...
          >
            <tbody>
              ...
              <transition-group name="list">
                <business-trip
                  v-show="!isFilter"
                  v-for="businessTrip in businessTripsdata"
                  :key="businessTrip.id"
                  :businessTripsdata="businessTrip"
                  @click="openModal(businessTrip)"
                ></business-trip>

                <business-trip
                  v-show="isFilter"
                  v-for="businessTrip in filterbyPage"
                  :key="businessTrip.id"
                  :businessTripsdata="businessTrip"
                  @click="openModal(businessTrip)"
                >
...
import BusinessTrip from "../businesstrips/BusinessTrip";
...
//<p>итог: {{JSON.stringify(filterbyPage)}}</p>
  setup() {
    ...
    const filterData = (data) => {
      ...
      switch (data) {
        case "asc":
          filteredData.value = Array.from(EntireBusinessTripList.value).sort(
            (a, b) => {
