      ...
      <div class="py-8 md:py-4">
        ...
        <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-6 overflow-x-auto">
          <div class="flex justify-between">
            <div>
              <p class="text-3xl font-bold">Больничные листы</p>
            </div>
          ...
          >
            <tbody>
              ...
              <transition-group name="list">
                <sick-leave
                  v-show="!isFilter"
                  v-for="sickLeave in sickLeavesdata"
                  :key="sickLeave.id"
                  :sickLeavesdata="sickLeave"
                  @click="openModal(sickLeave)"
                ></sick-leave>

                <sick-leave
                  v-show="isFilter"
                  v-for="sickLeave in filterbyPage"
                  :key="sickLeave.id"
                  :sickLeavesdata="sickLeave"
                  @click="openModal(sickLeave)"
                >
...
import paginateSickLeavesList from "../../composables/PaginateUniversal";
import SickLeave from "../sickleaves/SickLeave";
...
//<p>итог: {{JSON.stringify(filterbyPage)}}</p>
  setup() {
    ...
    const filterData = (data) => {
      ...
      switch (data) {
        case "asc":
          filteredData.value = Array.from(EntireSickLeaveList.value).sort(
            (a, b) => {
